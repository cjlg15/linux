...
```scp```是```secure copy```的简写，用于在Linux下进行远程拷贝文件的命令，和它类似的命令有```cp```，不过```cp```只是在本机进行拷贝不能跨服务器，而且```scp```传输是加密的。可能会稍微影响一下速度。当你服务器硬盘变为只读 read only system时，用```scp```可以帮你把文件移出来。另外，```scp```还非常不占资源，不会提高多少系统负荷，在这一点上，```rsync```就远远不及它了。虽然 ```rsync```比```scp```会快一点，但当小文件众多的情况下，```rsync```会导致硬盘I/O非常高，而```scp```基本不影响系统正常使用。

1. 命令格式：
```scp [参数] [原路径] [目标路径]```
1. 命令功能：
```scp```是 secure copy的缩写, ```scp```是linux系统下基于ssh登陆进行安全的远程文件拷贝命令。linux的```scp```命令可以在linux服务器之间复制文件和目录。
1. 命令参数：
